...
### 2. 聊聊这个系列开篇就应该回答的问题：我们为什么需要实时数据可视化和分析的方案？ 
...
信息化社会的迅猛发展早早就造就了一系列强大的传统数据分析和商业智能分析工具的诞生，这些软件大多采用ETL（Extract, Transform, Load）的传统方法从数据库加载数据到工具的数据仓库中中再供用户进行操纵；而如今的工业级传感设备一般在每秒就能产生将近百万条的机器数据，数据产生频率和数据会造成转移过程中巨大的困难和延迟。  
工程师们为了使工具适应当今的业务情景，也想了很多补救办法，比如加速数据加载进程，手动地横向扩展数据转移过程等，但即使是这样，完成整个数据获取分析的过程也停留在分钟级别。   
有些公司想出了异步地将数据从数据仓库转移到分析仓库的方法来加速整个进程，而这个方法确实能将数据可供分析的进程提高到近乎实时的表现。只不过这种表现下频繁地请求数据将对分析仓库造成巨大的压力。 因此大部分的传统数据分析工具对实时数据可视化和分析的支持非常不好，或者说甚至没有。 
